...
export default function PlayerDetailPage() {
  ...
  const { data: player, isLoading } = usePlayer(Number(params.id))
  ...
  if (!player) {
    return (
      <div className="space-y-4">
        <Button variant="ghost" onClick={() => router.push('/squad')}>
  ...
  return (
    <div className="space-y-6">
      <Button variant="ghost" className="-ml-2" onClick={() => router.push('/squad')}>
        <ArrowLeft className="h-4 w-4 mr-2" /> Back to Squad
      </Button>
